...
exports.get = function(req, id, client, callback) {
  client.hgetall('rollerpost:' + id, function(err, post) {
    if (err) {
      ...
      var isLiked = false;
      var isRepostByYou = false;
      var isRepostByOther = false;
      ...
      // Is this something you reposted?
      if (post.isRepost) {
        if (post.author === req.session.email) {
          isRepostByYou = true;
        } else {
          isRepostByOther = true;
        }
      }
      ...
      client.sismember('rollerposts:set:likes:' + req.session.email, id, function(err, resp) {
        if (err) {
          ...
          post.isRepostByYou = isRepostByYou;
          post.isRepostByOther = isRepostByOther;
